...
function XPFloat({ show }: { show: boolean }) {
  return (
    <div style={{
      ...
      background: "#76AD25", color: "#ffffff", fontWeight: 800,
...
function SlideActivity({ a }: { a: Activity }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 800, fontSize: "clamp(1.2rem, 3vw, 1.6rem)", color: "#ffffff", marginBottom: 24, lineHeight: 1.3 }}>
        {a.title}
      </h2>
      <div style={{
        background: "rgba(255,255,255,.06)", borderRadius: 16,
        padding: "24px 28px",
        fontSize: "0.95rem", color: "#4a6a8a", lineHeight: 1.9,
        whiteSpace: "pre-wrap",
    ...
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      {questions.map((q, qi) => (
        <div key={qi} style={{ marginBottom: 32 }}>
          <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.3rem)", color: "#ffffff", marginBottom: 20, lineHeight: 1.4 }}>
            {q.question}
          </h2>
          ...
          {locked[qi] && q.explanation && (
            <div style={{
              ...
              fontSize: "0.875rem", color: "#4a6a8a", lineHeight: 1.6,
...
function TrueFalseActivity({ a, onPass, onWrong }: { a: Activity; onPass: () => void; onWrong: () => void }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.4rem)", color: "#ffffff", marginBottom: 32, lineHeight: 1.4 }}>
        {a.statement}
      </h2>
      ...
      {locked && a.explanation && (
        <div style={{ marginTop: 16, padding: "14px 18px", borderRadius: 12, background: answered === correct ? "rgba(118,173,37,.12)" : "rgba(239,68,68,.1)", border: `1px solid ${answered === correct ? "rgba(118,173,37,.3)" : "rgba(239,68,68,.25)"}`, fontSize: "0.875rem", color: "#4a6a8a", lineHeight: 1.6, animation: "fadeIn 0.3s ease" }}>
          {a.explanation}
        </div>
      )}
    </div>
...
function DragDropActivity({ a, onPass, onWrong }: { a: Activity; onPass: () => void; onWrong: () => void }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.3rem)", color: "#ffffff", marginBottom: 8, lineHeight: 1.4 }}>
      ...
      {!checked && placed.length > 0 && (
        <button onClick={check} style={{ width: "100%", padding: "14px", background: "#76AD25", color: "#ffffff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", fontFamily: FONT }}>
          Check Answer
        </button>
      )}
      {checked && correct && (
...
function TypedActivity({ a, onPass, onWrong }: { a: Activity; onPass: () => void; onWrong: () => void }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.35rem)", color: "#ffffff", marginBottom: 28, lineHeight: 1.4 }}>
        {a.question}
      </h2>
      <input
        ref={inputRef}
        ...
        style={{
          ...
          borderRadius: 12, color: "#ffffff", fontFamily: FONT, fontSize: "1rem",
      ...
      {result === null && (
        <button onClick={submit} disabled={!value.trim()} style={{ width: "100%", padding: "14px", background: value.trim() ? "#76AD25" : "rgba(255,255,255,.08)", color: "#ffffff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: "0.9rem", cursor: value.trim() ? "pointer" : "not-allowed", fontFamily: FONT }}>
          Submit
        </button>
      )}
      {result === "correct" && (
...
function VocabActivity({ a, vocab }: { a: Activity; vocab: { term: string; definition: string }[] }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.3rem)", color: "#ffffff", marginBottom: 6 }}>{a.title ?? "Vocabulary"}</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
        {vocab.map((v, i) => (
          <div key={i} onClick={() => setFlipped(s => { const n = new Set(s); n.has(i) ? n.delete(i) : n.add(i); return n; })} style={{ background: flipped.has(i) ? "rgba(118,173,37,.12)" : "rgba(255,255,255,.06)", border: `1.5px solid ${flipped.has(i) ? "rgba(118,173,37,.4)" : "rgba(255,255,255,.1)"}`, borderRadius: 14, padding: "18px", cursor: "pointer", minHeight: 100, display: "flex", flexDirection: "column", justifyContent: "center", transition: "all .2s" }}>
            {flipped.has(i) ? (
              <p style={{ color: "#4a6a8a", fontSize: "0.875rem", lineHeight: 1.6 }}>{v.definition}</p>
            ) : (
              <div style={{ fontWeight: 700, fontSize: "1rem", color: "#ffffff" }}>{v.term}</div>
            )}
          </div>
        ))}
      </div>
...
function ScenarioActivity({ a }: { a: Activity }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.3rem)", color: "#ffffff", marginBottom: 20 }}>{a.title}</h2>
      <div style={{ background: "rgba(255,255,255,.06)", borderRadius: 14, padding: "20px", marginBottom: 16, border: "1px solid rgba(255,255,255,.08)" }}>
        ...
        <p style={{ color: "#4a6a8a", fontSize: "0.9rem", lineHeight: 1.75, whiteSpace: "pre-wrap" }}>{a.setup}</p>
      </div>
      <div style={{ background: "rgba(59,130,246,.08)", borderRadius: 14, padding: "20px", marginBottom: 16, border: "1px solid rgba(59,130,246,.2)" }}>
        ...
        <p style={{ color: "#4a6a8a", fontSize: "0.9rem", lineHeight: 1.75, whiteSpace: "pre-wrap" }}>{a.challenge}</p>
      </div>
      {(a.hints ?? []).length > 0 && (
      ...
      {showSolution && a.solution && (
        <div style={{ background: "rgba(118,173,37,.08)", border: "1px solid rgba(118,173,37,.2)", borderRadius: 14, padding: "20px", animation: "fadeIn 0.3s ease" }}>
          ...
          <p style={{ color: "#4a6a8a", fontSize: "0.875rem", lineHeight: 1.75, whiteSpace: "pre-wrap" }}>{a.solution}</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <div style={{ background: "rgba(255,255,255,.06)", borderRadius: 14, padding: "20px", marginBottom: 20, border: "1px solid rgba(255,255,255,.08)" }}>
        <p style={{ color: "#4a6a8a", fontSize: "0.9rem", lineHeight: 1.75, whiteSpace: "pre-wrap" }}>{a.prompt}</p>
      </div>
      <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Write your thoughts here..." rows={5} style={{ width: "100%", background: "rgba(255,255,255,.06)", border: "1.5px solid rgba(255,255,255,.12)", borderRadius: 12, padding: "16px", color: "#ffffff", fontFamily: FONT, fontSize: "0.9rem", resize: "vertical", outline: "none" }} onFocus={e => e.target.style.borderColor = "#76AD25"} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,.12)"} />
...
function DiscussionActivity({ a }: { a: Activity }) {
  ...
  return (
    <div style={{ animation: "slideIn 0.3s ease-out" }}>
      ...
      <h2 style={{ fontWeight: 700, fontSize: "clamp(1rem, 2.5vw, 1.3rem)", color: "#ffffff", marginBottom: 20 }}>{a.title ?? "Discussion"}</h2>
        ...
        <div>
          ...
          {a.guidingQuestions!.map((q, i) => (
            <div key={i} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
              ...
              <p style={{ color: "#4a6a8a", fontSize: "0.875rem", lineHeight: 1.6 }}>{q}</p>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{
      ...
      <div style={{ opacity: showContent ? 1 : 0, transform: showContent ? "translateY(0)" : "translateY(30px)", transition: "all 0.5s ease", maxWidth: 480, width: "100%" }}>
        ...
        <h1 style={{ fontWeight: 900, fontSize: "2rem", color: "#ffffff", marginBottom: 6, lineHeight: 1.2 }}>
          {lesson.title}
        </h1>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <button onClick={onContinue} style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 32px", background: "#76AD25", color: "#ffffff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: "1rem", cursor: "pointer", fontFamily: FONT, boxShadow: "0 4px 20px rgba(118,173,37,.4)" }}>
...
function LessonContent() {
  ...
  if (!lesson) return (
    <div style={{ minHeight: "100vh", background: "#0a1628", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      ...
      <button onClick={() => router.push("/curriculum")} style={{ padding: "10px 20px", background: "#76AD25", color: "#ffffff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: FONT, fontWeight: 700 }}>
        Back to Curriculum
      </button>
    </div>
  );
